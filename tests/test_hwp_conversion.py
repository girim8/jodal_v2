...
def test_extract_text_from_hwpx_bytes():
    from io import BytesIO
    import zipfile

    payload = BytesIO()
    with zipfile.ZipFile(payload, "w") as zf:
        zf.writestr("contents.xml", "<root>계약 번호 스쿨넷</root>")
    text, status = hwp_utils.extract_text_from_hwpx_bytes(payload.getvalue())
    assert "계약" in text
    assert status == "OK[HWPX]"
