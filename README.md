...
## 3. HWP 추출 품질 점검
새로 추가된 CLI로 현장에서 바로 검증할 수 있습니다. `.hwp`뿐 아니라 `.hwpx`도 지원하며, 기본 키워드(`계약`, `번호`, `금액`)만으로도 한글 추출 품질을 빠르게 점검합니다. 저장소 루트에 제공된 샘플 파일(`공고서_국가_일반_국내_유선통신서비스_1458+0036_공동이행_하도급사전승인_장기계속계약_차등점수제_수요기관평가.hwp`)을 사용할 경우 **결과가 `계약 O / 번호 O / 금액 X`**로 표시되는 것이 정상입니다.
```bash
...
- `--fail-missing` : 필수 키워드가 모두 나오지 않으면 종료 코드 2로 실패 처리합니다. (샘플 파일은 `금액`이 누락되므로 기본 제공 예시에는 포함하지 않았습니다.)
...
- 저장소 루트에 샘플 HWP가 있으면 `scripts/hwp_inspector.py --json` 결과를 기반으로 **`계약 O / 번호 O / 금액 X`** 상태까지 자동 검증합니다.
